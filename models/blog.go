...
type Blog struct {
	...
	ReadNum    int       `gorm:"size:8;default:0;" json:"read_num"`        // 阅读数量
	CommentNum int       `gorm:"size:8;default:0;" json:"comment_num"`     // 评论数量
	LikeNum    int       `gorm:"size:8;default:0;" json:"like_num"`        // 点赞数量
	CollectNum int       `gorm:"size:8;default:0;" json:"collect_num"`     // 收藏数量
	IsComment  bool      `json:"is_comment"`                               // 是否开启评论
	IsPublish  bool      `json:"is_publish"`                               // 是否发布
	IsTop      bool      `json:"is_top"`                                   // 是否置顶
	TopTime    time.Time `json:"top_time"`                                 // 置顶时间
	CategoryID uint      `json:"category_id" gorm:"foreignKey:CategoryID"` // 分类ID
